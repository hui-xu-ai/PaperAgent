...
- **中文译文里的引用上标退化成字面 `[[66]]`**（2026-09-23 用户报障"两种修复代码明显不兼容"，
  实测 `kb/10.1002_adma.202407106` 中文侧 6 处、全库 9 处）：
  - **根因**：模型把引用上标写成**既无 `^` 也无 `$`** 的双括号 `[[66]]`（模仿 app 自身的
    双链 / `[[MATHn]]` 占位符语法）。它落在两套修复的**夹缝**里：① 上标归一
    `normalize_citation_superscripts` 只认 `^[` ⇒ 认不出它；② 阅读器 `reader.js` 的 wikilink
    规则把 `[[66]]` 当 **Obsidian 双链**渲染成链接。**两种机制对 `[[n]]` 的语义判断相反**
    —— 这正是用户看到的"不兼容"。
  - **不是配平代码改坏的（取证）**：09-21 备份该段是 6 处 `$^{[n]}$`（14 个 `$`、6 个 `^`）；
    现版是 6 处 `[[n]]`、`^` 计数 **0**；而三步归一链对**两版都是 no-op**（改动=False），
    且全库无任何代码匹配/删除 `^[` ⇒ 插入符不是被代码吃掉的（是新一轮翻译的模型输出换了形态）。
  - **修法**：在**同一个** `normalize_citation_superscripts` 里追加无 `^` 形态
    （`[[66]]` → `^{[66]}`），随后的 `wrap_bare_scripts` 照旧包成 `$^{[66]}$` ⇒ **全链只有一个语义**。
    一处改动自动覆盖三个既有调用点（翻译写回 / `sanitize_document` 源头 / `_clean_html` 渲染兜底），
    **不为它新开机制**（"两套修复打架"的教训）。
  - **与 app 自身语法零交集（全库实测）**：只认"纯数字/引用分隔符 + `]]` 紧贴 body" ⇒
    `[[DOI/_note|笔记]]`、`[[66/_note|笔记]]`、`[[66|别名]]`、`[[MATH12]]`、`[[笔记]]`
    归一改动均为 **0**；已在上标花括号内的 `$^{[[66]]}$` 也不碰（避免套成 `$^{^{[66]}}$` 非法嵌套）。
  - 全库影响面：**只有 adma 的 4 个文件、共 9 处**（library / kb 的 document.json +
    kb 的 `zh.md` / `en_zh.md`），无其它连带。
  - 测试：`test_normalize_bare_double_brackets`（4 例，含区间/并列 + 全链 + 幂等）、
    `test_bare_double_bracket_no_false_positive`（7 例，全是 app 自身语法与边界）。
  - ⚠️ 已入库的历史译文：**重翻译**即修好；已渲染的 `zh.md`/`en_zh.md` 走渲染层兜底
    （换模板/重解析会重渲染变体）。**在兜底渲染之前**，旧文件里的 `[[66]]` 仍会被阅读器
    显示成链接——这是显示层对干净数据的依赖，不是在数据里留下了新的坏形态。
