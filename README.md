...
**整机界面**：左＝对话与知识库问答（编译产物直接可读、可就地追问），右＝原文 / 译文 / 图片 / 笔记多标签阅读器，
可切 原文 / PDF / 双语对照 / CN 中文 / 图片 / 笔记(L1·L2·L3)；顶部导航直达 知识库管理 / AI 检索 / 文献图谱 / 设置；
底部状态栏实时显示 token 用量与缓存命中。

![整机界面](assets/screenshots/文献AI阅读功能.png)

**知识库与 AI 检索**：知识库标签页按 编译进度(L1/L2/L3) / 期刊 / 年份 / 价值分 管理文献，
用一句话提问即可检索并作答——回答里带 DOI、作者、年份、期刊、摘要要点，可点回原文核对。

![知识库问答与 AI 检索](assets/screenshots/AI检索功能.png)

**文献计量图谱**：库内文献的 2D 平面 / 3D 立体引用网络，节点按影响因子着色、大小按库内被引，
右侧详情面板给出 DOI / 分区 / IF / 被引次数。
