...
**整机界面**：左＝会话 + 文献库（类型芯片/搜索语法糖/附件入口），中＝阅读与笔记（编译产物直接可读、可就地追问），
右＝原文 / 译文 / 图片 / 笔记多标签阅读器；底部状态栏实时显示 token 用量与缓存命中。

![GUI 总览](assets/screenshots/gui-overview.png)

**文献阅读**：PDF 原文与中文译文并排（图片自动抽出可单独浏览），公式/表格保留，可切 原文 / 双语对照 / 中文 / 图片 / 笔记(L1·L2·L3)。

![文献阅读](assets/screenshots/reading.png)
