...
5. **核对**：窗口里打开「设置 → 关于」，应看到 **应用版本 / 界面版本**（两者必须相同，不同会红字报警）
   ...
   ```json
   {"status":"ok","version":"1.4.0","data_format":4, ...}
   ```
   `version` = 新程序版本；`data_format` = 数据格式（升级后可能变大，正常，见 §4 矩阵）。
...
**矩阵（`data_format` 到底变了什么）**

| `data_format` | 首次随此版本发布 | 该格式对数据的改动 | 要重新解析 PDF 吗 |
|---|---|---|---|
| **4** | **1.4.0（当前）** | `papers_meta` 纯增列：AI 评分 2 列 + 文献元数据 6 列 | 不用 |
| 3 | 1.3.0 | `chat.messages` 增 `reasoning_content` 列 | 不用 |
| 2 | 1.0.0 | `settings.prices` 结构规范化；`papers_meta` 主键 `doi→rid` | 不用 |
| 1 | 1.0.0 之前 | 旧单库/内联补列时代，升到任意新版时由迁移层一次性补齐 | 不用 |

> 一栏读懂：**`data_format` 变大只表示"表结构比上一版多"，不等于"你的数据要重做"**。
> 升级路径永远是"退出程序 → 覆盖程序文件 → 首启自动备份 + 迁移"（见 §2）。
