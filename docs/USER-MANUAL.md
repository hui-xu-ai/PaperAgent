...
- **主模型**：`设置 → 🤖 模型 → AI模型`；
- **翻译专用模型**（可选）：同一页 **`翻译模型`** → `添加翻译模型`。界面原话：「启用 1 个=切换，启用多个=并行轮询提速，都不启用=用主模型」；
...
| 页 | 关键项 |
|---|---|
| **`🤖 模型`** | `AI模型`（列表 + `＋ 新建` / `编辑` / `删除` / `激活`，弹窗内 `测试连接`）；`翻译模型`（可选池，启用多个＝并行提速） |
...
| **`📚 知识库`** | `知识库路径`（留空＝项目内 `knowledge_base/`）；生成方式（`复制副本（默认）` / `硬链接`）；`文献会话检索范围`；**编译/翻译思考档位** |
