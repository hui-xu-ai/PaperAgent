...
- **单批上限**：**这个值 = 一次请求最多装多少「英文原文」字符**（含空格，按 `len(原文)` 数；**不是中文字数，也不是 token**）。它决定"一篇论文会分成几次请求"，不决定译文质量。
  - 在**每个翻译模型条目里各设一份**（`翻译模型 → 编辑 → 单批上限`）——因为"这个模型一次能安全译多少字"是**模型自己的属性**，换模型就该用它自己的值。留空＝用默认（专用模型 14000 / 主模型 12000）。
  - 举例读法：填 `14500` ⇒ 一次请求发出约 14500 个英文字符（≈3500 输入 token、译文约需 2900 输出 token）；一篇 10 万字符的论文会拆成约 7 批。**中文字数远少于这个数**（中文比英文短），别把 14500 当成"译文 14500 字"。
  - 分批**只在段落边界切，绝不切断句子**；单段超过上限时该段整段单独发送（不与其他段合并）；
  - 调大 → 调用次数更少更快，但超出模型输出能力会截断（截断会自动按单段补跑，不漏译，只是白花 token）；
  - 怎么定：**别猜，点条目里的 `🔬 自动测一个值`**（见下），测出的值会**自动填进这个框**；
  - 手动办法：先留空用默认跑一篇，看日志里有没有 `批截断`。有，就按日志给出的建议值往小调（日志会写成
    `批截断：3/12 段未译（本批 14120 字符，上限 14000），按单段补跑；若频繁出现，请把「翻译批次上限」降到 ≤7060`）。
- **`🔬 自动测一个值`**（在翻译模型编辑弹窗里，挨着 `测试连接`）：用这个模型真实翻译几段，测出它**一次能完整译出多少字**，并自动回填上面的框（**点保存才随该模型生效**）。
  - **测谁**：就测你正在编辑的这个模型（不必先激活、也不必先保存），用的是框里填的 Base URL/模型/Key；
  - **测什么**：拿你库里**真实论文的英文正文**（知识库 → 解析库，按 `document.json` 体积从大到小取，最多 30 篇；取段口径与真实翻译同源 ⇒ 自动跳标题与参考文献；优先挑长段以贴近真实批次形状，整段取用、绝不切句）；**库里还没有已解析的论文时无法测**——会提示"请先解析或导入至少一篇文献"，此时留空用默认值即可（默认值对出厂翻译模型已实测安全，导入后再测更准）；
  - **怎么测**：按 3000 → 6000 → 12000 → 24000 → 48000 字符**逐档真译一次**，通过就加码；
  - 测完会告诉你**用了哪几篇、语料中位段长**（"你这批语料单批最多约 N 字符"），所以你知道这个值是在什么语料上量出来的；
  - **怎么判失败**：供应商自报 `finish_reason=length`（输出触顶）**或** 该回的段落没回全（思考型模型常不报截断），任一命中即算该档失败；**第一档失败就停**（更大的档不可能反而通过），所以通常只跑 2~3 次；
  - **建议值不是测试极限**：建议值 = 最高通过档 × **0.6**（留 40% 余量，避免"这次过了下次就断"）；
  - **代价**：最多 5 次真实翻译调用（累计输入约 0.9~12 万字符，计入用量），可能要几分钟；跑的时候可以关设置窗（后台继续跑）；
  - **弹窗里能看到全过程**（2026-09-23 重设计）：编辑翻译模型的弹窗是**宽版**，下方有一块 **`操作记录`**，逐步记下「开始探测 → 第几档 → 每档通过/失败 → 建议值 → 已保存（以后端回读值为准）」。两点防误会的约束：
    - **探测期间 `保存` 被禁用**（状态行会写明）——避免在建议值回填**之前**就把空值存进去；
    - **关窗后后台继续跑**：结果出来时若弹窗已关，记录里会明确写"该值还没保存"；**表格的 `单批上限` 列**会显示 `默认（建议 N）`，重新打开这一条会**自动回填**，再点 `保存` 才生效；
  - 若面板写着"未测出可用值"且提示 `max_tokens`：说明**最低档就撞上了"请求的输出预算"**——这个模型输出远比预期啰嗦（例如思考型模型把推理链也算进输出）。程序会把该预算随上限**自动放大**（预算 = max(该模型已配值, 上限 × 0.4)），所以先把上限调大一档再重测；仍失败就换一个输出更强的模型。
- **知识库页那个「单批上限（默认值）」**：只在**两个场合**生效——① 某个模型条目自己没设；② 回落主模型翻译时。想按模型精调就填在模型条目里（条目值优先）。
- **输出上限（max_tokens）不用你填**：翻译专用模型的这个值已从表单移除，程序按上面的公式由单批上限自动推导（只增不减，绝不改动主模型）；实测输出 token ≈ 源字符 × 0.2，14000 字符批次约 2800 token，默认余量充足。
- **也可以直接在 `.env` 里配翻译模型**（与 Key 同一规则：**`.env` 是权威，手改即生效**）：每槽一组 `TRANSLATE_<序号>_BASE_URL / _MODEL / _API_KEY / _ENABLED / _MAX_TOKENS / _BATCH_CHARS`（序号从 0 起，`_ENABLED=1` 即激活）；界面上保存翻译模型时也会回写到 `.env`，两边始终一致。
...
| **L1** | 知识笔记 | `_note.md`（一句话贡献 + 六维笔记 + 核心概念（含定义）+ 概念标签） | **入库自动触发** |
...
| 页 | 关键项 |
|---|---|
| **`🤖 模型`** | `AI模型`（列表 + `＋ 新建` / `编辑` / `删除` / `激活`，弹窗内 `测试连接`）；`翻译模型`（可选池：可存多个备选，**单选激活**其中一个；都不激活＝用主模型；每条可单独设**单批上限**，弹窗内 `🔬 自动测一个值` 自动测出并回填） |
...
| **`📚 知识库`** | `知识库路径`（留空＝项目内 `knowledge_base/`）；生成方式（`复制副本（默认）` / `硬链接`）；`文献会话检索范围`；**编译/翻译思考档位**；**单批上限（默认值）**（字符，留空＝默认；每个翻译模型可在其条目里单独设，条目值优先） |
...
> **`.env` 是配置的唯一权威来源**（2026-09-22 起）：程序启动时会用 `.env` 里的值**覆盖系统环境变量**
> （空行也算——在 `.env` 里把某行清空，该配置就真的失效）。所以**只改这一个文件就够了**，
> 不要去设 Windows 环境变量，否则会出现"我在 `.env` 里改了却不生效"。启动日志里若出现
> `以下环境变量被 .env 覆盖…`，就是在告诉你哪些键原本被系统变量压住了。
> 界面里保存的 Key 也会写回 `.env` 并**立即生效**（不需要重启）。
> **同一个模型出现两份 Key 时（`.env` 一份、界面供应商条目里一份），以 `.env` 为准**（2026-09-23 起）：
> 例如你在 `.env` 里换了 `ZHIPU_API_KEY`，即使界面上那条「智谱」供应商里还存着旧 Key，程序也会用
> `.env` 的新 Key——启动日志里会有一行 `供应商「智谱」的 Key 与 .env 不一致 → 已按 .env 覆盖`。
> 反之，**在界面上改了 Key 并保存**，会同步写回 `.env`，两边不会再分叉。
...
| 编译失败（卡片显示失败原因） | 失败**不会自动重试**（旧行为是每 5 秒重试一次、永不放弃，会把日志刷满），失败原因就地显示；处理完原因后点 `批量重试编译` |
| 报错 `HTTP 401 Unauthorized` | 该模型的 Key 无效/过期。编译走**主模型**（`设置 → 🤖 模型` 里激活的那个），翻译走翻译模型——两者 Key 分开检查 |
| 日志刷 `engine 第 N 次（上限 12，红线）` | 这是**同一个失败任务被反复重试**在计数（不是有人在狂调模型）。2026-09-23 起：编译有独立额度且失败即停，不会再出现这种刷屏 |
