...
- **`🔬 测试安全上限`**（在批次上限下面那个按钮）：自动测出"当前翻译模型一次能完整译出多少字"，填不填由你点。
  ...
  - **只给建议**：面板上的 `写入 N 字符` 按钮你点了才写（写入即用既有接口落盘生效）；
  - **代价**：最多 5 次真实翻译调用（累计输入约 0.9~12 万字符，计入用量），可能要几分钟；跑的时候可以关设置窗（后台继续跑，进度会丢但结果会存下来）；
  - **超 30 天**会提示重测（模型/服务端输出上限可能已变）；
  - 若面板写着"未测出可用值"且提示 `max_tokens`：说明**最低档就撞上了"请求的输出预算"**——这个模型输出远比预期啰嗦（例如思考型模型把推理链也算进输出）。程序会把该预算随「翻译批次上限」**自动放大**（预算 = max(该模型已配值, 上限 × 0.4)），所以先把批次上限调大一档再重测；仍失败就换一个输出更强的翻译专用模型。
- **输出上限（max_tokens）不用你填**：翻译专用模型的这个值已从表单移除，程序按上面的公式由「翻译批次上限」自动推导（只增不减，绝不改动主模型）；实测输出 token ≈ 源字符 × 0.2，14000 字符批次约 2800 token，默认余量充足。
...
| **`📚 知识库`** | `知识库路径`（留空＝项目内 `knowledge_base/`）；生成方式（`复制副本（默认）` / `硬链接`）；`文献会话检索范围`；**编译/翻译思考档位**；**翻译批次上限**（字符，留空＝默认）+ **`🔬 测试安全上限`**（自动测出建议值，点 `写入` 才生效） |
