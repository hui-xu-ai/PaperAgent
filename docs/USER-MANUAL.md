...
| 页 | 关键项 |
|---|---|
| **`🤖 模型`** | `AI模型`（列表 + `＋ 新建` / `编辑` / `删除` / `激活`，弹窗内 `测试连接`）；`翻译模型`（可选池：可存多个备选，**单选激活**其中一个；都不激活＝用主模型；每条可单独设**单批上限**，弹窗内 `🔬 计算单批上限` 自动测出并回填） |
