...
2026-09-23 用户报障：中文译文里同一件事（文献引用上标）有**四种**写法，读者看到的效果完全不同。
实测（`kb/10.1016_j.cej.2025.167798`、`kb/10.1002_adma.202407106`）：
  ① `$^{[29]}$`            —— 规范（37 处）；
  ...
  ④ `[[66]]`               —— **既无 `^` 也无 `$`**（实测 adma 篇 6 处）。模型模仿 app 自身的
                              双链 / `[[MATHn]]` 占位符语法写引用；阅读器又会把它当 **Obsidian
                              双链**渲染成链接 ⇒ 与"上标"语义冲突（用户报"两种修复不兼容"）。
...
# ★2026-09-23 追加（用户报障"上标修复被干扰"，实测 `kb/10.1002_adma.202407106`）：
# **既无 `^` 也无 `$`** 的双括号 `[[66]]`。模型把引用上标写成双括号（模仿 app 自身的双链
# / `[[MATHn]]` 占位符语法），于是落进两套修复代码的**夹缝**：
#   ① `normalize_citation_superscripts` 只认 `^[` ⇒ 认不出它；
#   ② 阅读器把 `[[66]]` 当 **Obsidian 双链**渲染成链接（`reader.js` 的 wikilink 规则）。
#   两种修复对 `[[n]]` 的语义判断不一致 —— 这就是"不兼容"的根源。
#   归一成 `^{[n]}`（随后同一套 `wrap_bare_scripts` 包成 `$^{[n]}$`），全链只有一个语义。
# 与 app 自身双链**无交集**：只认"纯数字/引用分隔符"，且要求 `]]` 紧贴 body
# （`[[DOI/_note|笔记]]` 含字母与 `|`、`[[66|别名]]` 带别名、`[[MATH12]]` 含字母 ⇒ 均不匹配）。
_CITE_BARE_DBL_RE = re.compile(r"(?<![\^\{])\[\[(" + _CITE_BODY + r")\]\]")
...
def normalize_citation_superscripts(text: str) -> tuple[str, int]:
    """`^[[38]]` / `^[38]` / `[[38]]` → `^{[38]}`；返回 (新文本, 修了几处)。**幂等**。

    只动"内容是数字/引用分隔符"的方括号上标 —— 真正的内联脚注（如 `^[见附录]`）不碰，
    app 自身双链（`[[目录/_note|笔记]]`）也不碰。先处理带 `^` 的形态、再处理无 `^` 的双括号。
    """
    ...
    out = _CITE_SUP_RE.sub(_repl, text)
    return _CITE_BARE_DBL_RE.sub(_repl, out), fixed
