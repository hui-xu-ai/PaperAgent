...
# 2026-09-19 曾判「输出侧是硬瓶颈」——混元 MT-7B 每批 ~1900 token 即截断、Qwen 4134 截断，
# 当时归因于「服务端/模型硬上限，max_tokens 请求参数无法覆盖」。
# 2026-09-23 实测**推翻该归因**（Qwen2.5-7B-Instruct via 硅基流动，读真实 usage 计数）：
#   输出 token ≈ 源英文字符 × 0.20（旧估「1:1」偏高约 5 倍）；输入 ≈ 源字符 × 0.24；
#   耗时 ≈ 24 ms/输出 token（≈41 tok/s），**与批次大小无关**（总时长正比于总输出量）。
#   48K 字符档失败的 completion_tokens 恰好 == 请求里的 max_tokens(8192)，分毫不差
#   ⇒ 截断来自**请求侧输出预算**，不是模型硬顶。故给足 max_tokens 后上限可显著放大：
#   6000 / 12000 / 24312 字符三档全数回全，49515 触顶。
# 取值 14000 字符：≈2800 输出 token，对 8192 预算留 ~2.9 倍余量；出厂默认翻译模型即 Qwen2.5-7B。
# ⚠ 换更弱/更小的模型请先测：`🔬 测试安全上限` 会测出该模型自己的值并写入「翻译批次上限」
#   （用户设置优先于本常量）。若该模型真有更小的服务端硬顶，也会被探测如实测出。
# 段数 ≤12（过多段 JSON 结构开销增大）。超长段落（>COMPACT_UNIT_MAX）走句子切块。
# 完整性保证：_make_batches 只整段入批（size+len(t)>max_body 即开新批，不切段中）；
# 超长单段（>COMPACT_UNIT_MAX）走 _split_sentences 按句切块；触顶另有「应译未译单段补跑」兜底。
COMPACT_MAX_BODY_CHARS = 14000
