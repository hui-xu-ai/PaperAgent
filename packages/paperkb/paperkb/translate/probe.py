...
#: 阶梯档位（正文字符数）。首档 3000 已小于紧凑默认 6000 ⇒ 首档就失败是有诊断价值的信号。
...
def probe_safe_batch_chars(llm, paras: list[str], *, ladder: tuple[int, ...] = LADDER,
    ...
    if not recommended:
        if stop_reason == "insufficient_text":
            ...
            hint = ("探测调用报错（多为超时/限流），未测出安全值；稍后重试，或先把「输出上限 "
                    "max_tokens」（当前 %s）调大。" % (max_tokens or "未设置"))
        else:
            hint = ("最低档 %d 字符就失败 ⇒ 瓶颈不是批次上限，而是**输出上限 max_tokens**（当前 %s）"
                    "或该模型输出能力。请先把 max_tokens 调大（或换输出更强的模型）再重测。"
