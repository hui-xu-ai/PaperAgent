...
7. **墙钟上限 = 生产客户端的读超时**（当前 150s，见 `TIER_TIMEOUT_SEC`；2026-09-23 用户报
   "卡在第 5 档"与"多次重试超时"后两次收紧，当日再由用户从 90s 提到 150s）：生产请求是
   **非流式**的，`requests` 的 read timeout 就等于"整批必须在 N 秒内返回"。
...
#: "生产读超时内根本回不来的批次"判为**通过** ⇒ 推荐值直接踩在生产超时上：实测用户那篇 14500 字符
#: 档，13852 字符的批连续 3 次 `Read timed out (read timeout=90)`（270s 白等 + 3 次输入白烧）才
#: 回落主模型；同一个模型拿 3777 字符的小批立刻成功 ⇒ 是**时间不够**，不是模型不会译。
#: 150 = `backend/app/services/llm_service.TRANSLATE_TIMEOUT_SEC`（专用翻译模型客户端；2026-09-23
#: 用户从 90s 提到 150s，给思考型/大档留出余量），
#: 两者由 `backend/tests/test_translate_timeout_contract.py` 守卫同步。
TIER_TIMEOUT_SEC = 150.0
...
#: 5 档 × 150s（= 单档墙钟）+ 余量：最坏情况（5 档全跑满）也够走完阶梯给出结论，
#: 正常 2-3 档就结束，不会把界面挂住。
